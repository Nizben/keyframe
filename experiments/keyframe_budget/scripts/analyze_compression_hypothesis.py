...
def main() -> None:
    ...
    for target in metric_targets:
        ...
        for predictor in PREDICTORS:
            ...
            if len(data) < 3:
                ...
                p = float("nan")
            corr_rows.append({"target": target, "predictor": predictor, "n": len(data), "spearman": rho, "p_value": p})
    ...
    for target in ["delta_temp", "delta_qual"]:
        for predictor in PREDICTORS:
            for k in (4, 5, 10):
                topk_rows.append({"target": target, "predictor": predictor, "k": k, "mean_overlap": mean_topk_overlap(df, predictor, target, k)})
    write_csv(args.out_dir / "topk_overlap.csv", topk_rows)

    bad_rows = []
    bad_df = df[["D_max", "Q", "delta_temp"]].replace([math.inf, -math.inf], math.nan).dropna().copy()
    if not bad_df.empty and len(set(bad_df["delta_temp"] < 0)) == 2:
        d_std = bad_df["D_max"].std(ddof=0) or 1.0
        q_std = bad_df["Q"].std(ddof=0) or 1.0
        bad_df["lowD_highQ"] = (bad_df["Q"] - bad_df["Q"].mean()) / q_std - (bad_df["D_max"] - bad_df["D_max"].mean()) / d_std
        labels = list((bad_df["delta_temp"] < 0).astype(int))
        auc = auroc_binary(labels, list(bad_df["lowD_highQ"]))
    else:
        auc = float("nan")
    bad_rows.append({"label": "delta_temp_negative", "predictor": "z(Q)-z(D_max)", "n": len(bad_df), "auroc": auc})
